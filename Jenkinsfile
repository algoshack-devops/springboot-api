      ...
      node(label) {
        ...
        stage('Create Docker images') {
          container(name: 'kaniko', shell: '/busybox/sh') {
            ...
            sh """
              ...
              /kaniko/executor -f `pwd`/Dockerfile -c `pwd` --destination=algoshack/springboot:${IMAGE_VERSION}
            """
            }
          }
        }
        ...
        if (env.BRANCH_NAME == 'master') {
          stage("Kubernetes") {
            container('kubectl') {
              sh """
                kubectl apply -f https://raw.githubusercontent.com/algoshack-devops/springboot-api/master/infra/k8s-deployment.yaml --namespace devops
                kubectl apply -f https://raw.githubusercontent.com/algoshack-devops/springboot-api/master/infra/k8s-service.yaml --namespace devops
	          """
            }
          }
        }
